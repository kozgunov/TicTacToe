...
class TicTacToeApp:
        ...
        for i in range(4):
            for j in range(4):
                if i == 0 and j == 0:
                    ...
                    self.buttons[i][j] = tk.Button(self.master, text='', font=('Arial', 12), width=20, height=10, command=lambda i=i, j=j: self.propose_move(i, j))
    ...
    def toggle_mark(self, i, j):
        ...
        if current_mark == 'O':
            ...
            self.buttons[i][j] = tk.Button(self.master, text=fix, font=('normal', 12), width=20, height=10, command=lambda i=i, j=j: self.make_move(i, j))
            self.buttons[i][j].grid(row=i, column=j)
